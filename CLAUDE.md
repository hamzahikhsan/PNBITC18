...
**Status Figma design file (update 11 September 2026):** Sudah mulai dibangun. File baru "Awetin - UI Design" (fileKey `uamDapxiZkJMGkPZ1INLfE`, https://www.figma.com/design/uamDapxiZkJMGkPZ1INLfE) dibuat khusus untuk mockup UI (terpisah dari board FigJam brainstorming di atas). Isinya:

- Page **"00 — Components"**: 34 color variables (`Awetin Colors` collection, mode Light), 12 text styles (`Awetin/Display` s.d. `Awetin/Caption`, font Plus Jakarta Sans), 1 komponen `Button` (variant Primary/Secondary, property teks `Label`), 1 frame reusable `BottomNav` (5 tab: Beranda/Pesanan/FAB Scan/Tukang/Profil) — dipakai via `.clone()` bukan instance sungguhan, supaya gampang ubah tab aktif per layar.
- Page **"01 — MVP Backbone (Sisi Pengguna)"**: **16 dari 16 layar prioritas MVP (PRD Bagian 12) sudah selesai dibangun & divalidasi screenshot satu per satu** — Splash/Onboarding, Onboarding Pilih Peran, Login (nomor HP + keypad), Izin Lokasi, Home, Scan AI (mode deteksi kritis), Hasil Deteksi AI + Skor Kelayakan + Titik Triase (digabung 1 layar sesuai alur prototype), Direktori Tukang, Detail Profil Tukang, Konfirmasi Jadwal & Biaya Jasa Tetap, Invoice & Persetujuan, Status Pesanan Aktif, Pembayaran QRIS, Upload Bukti Servis + Rating, Dashboard Dampak Komunitas, dan 1 contoh Onboarding Tukang (Pilih Kategori Keahlian, didesain baru karena belum ada di prototype HTML).
- Konten & copy tiap layar disalin persis dari `Prototype/awetin-prototype.html` (bukan didesain ulang dari nol) — hanya layar Onboarding Tukang yang baru, dibuat konsisten dengan design system yang sama.
- **Belum dikerjakan:** layar-layar "nice-to-have" sisanya dari daftar 39 layar PRD Bagian 8 (Jual-Beli, Donasi, Daur Ulang, Riwayat, Tukang Favorit, sisi dashboard Tukang lengkap, dll) — sesuai PRD Bagian 12 ini boleh cukup 1-2 layar representatif atau dijelaskan konsepnya saja saat presentasi, tidak wajib full prototype.
- **Catatan teknis penting kalau lanjut isi Figma dari sini:** setiap frame auto-layout yang harus "hug" konten (lebar maupun tinggi) WAJIB di-set eksplisit — `layoutSizingHorizontal`/`layoutSizingVertical = 'HUG'` untuk child di parent auto-layout, ATAU `primaryAxisSizingMode`/`counterAxisSizingMode = 'AUTO'` untuk frame yang parent-nya `layoutMode: 'NONE'`. Kalau cuma salah satu axis yang di-set, sisi lainnya diam-diam nyangkut di default 100×100 dan bikin teks/badge kepotong tanpa error — ini penyebab hampir semua bug visual selama build sesi ini (selalu screenshot tiap section untuk ketahuan cepat).
